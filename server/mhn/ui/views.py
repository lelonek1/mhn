...
@app.route('/image/<hpname>/top_passwords.svg')
@login_required
def graph_passwords(hpname):
    clio=Clio()
    
    bar_chart = pygal.Bar(style=LightColorizedStyle,show_x_labels=True, config=PYGAL_CONFIG)
    bar_chart.title = "%s Top Passwords" % hpname.capitalize()
    clio=Clio()
    top_passwords =clio.hpfeed.count_passwords(clio.hpfeed.get_payloads({'limit':10000},{"channel":"%s.sessions" % hpname})[2])
    for password in top_passwords:
        bar_chart.add(password[0],[{'label':str(password[0]),'xlink':'','value':password[1]}])

    return bar_chart.render_response()

@app.route('/image/<hpname>/top_users.svg')
@login_required
def graph_users():
    clio=Clio()
    
    bar_chart = pygal.Bar(style=LightColorizedStyle,show_x_labels=True, config=PYGAL_CONFIG)
    bar_chart.title = "%s Top Users" % hpname.capitalize()
    clio=Clio()
    top_users =clio.hpfeed.count_users(clio.hpfeed.get_payloads({'limit':10000},{"channel":"%s.sessions" % hpname})[2])
...
@app.route('/image/<hpname>/top_combos.svg')
@login_required
def graph_combos():
    clio=Clio()
    
    bar_chart = pygal.Bar(style=LightColorizedStyle,show_x_labels=True, config=PYGAL_CONFIG)
    bar_chart.title = "%s Top User/Passwords" % hpname.capitalize()
    clio=Clio()
    top_combos =clio.hpfeed.count_combos(clio.hpfeed.get_payloads({'limit':10000},{"channel":"%s.sessions" % hpname})[2])
...
@app.route('/image/<hpname>/top_sessions.svg')
@login_required
def graph_top_attackers():
    clio=Clio()
    
    bar_chart = pygal.Bar(style=LightColorizedStyle,show_x_labels=True, config=PYGAL_CONFIG)
    bar_chart.title = "%s Top Attackers" % hpname.capitalize()
    clio=Clio()
    top_attackers =clio.session._tops('source_ip',10,honeypot=hpname)
    print top_attackers    
